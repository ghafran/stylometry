...
readings stay intact, and their share is recorded as ``supplied_frac``; units that are mostly
reconstruction are dropped.
"""
...
MIN_SCROLL_TOKENS = 100      # skip tiny fragments as works of their own
MAX_SUPPLIED = 0.5           # drop units where more than half of the letters are reconstructed
UNIT_TARGET = 25
...
def _record(text: str, **kw) -> dict | None:
    ...
    if not toks:
        return None
    rec = {
        "source": "etcbc_dss", "language": "hbo", "copyist": None, "has_gap": False, "duplicate_of": None,
