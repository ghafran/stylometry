...
SCOPES = {
    "all": lambda v: True,
    "sinaiticus": lambda v: v["source"] == "sinaiticus",
    "lxx": lambda v: v["collection"] == "LXX",
    "nt": lambda v: v["collection"] == "NT",
    "christian": lambda v: v["language"] == "grc" and v["collection"] != "LXX",
    "noncanonical": lambda v: v["language"] == "grc" and v["canon"] == "none",
    "greek": lambda v: v["language"] == "grc",
    "hebrew": lambda v: v["language"] == "hbo",
    "arabic": lambda v: v["language"] == "arb",
    "tanakh": lambda v: v["language"] == "hbo" and v["collection"] == "Tanakh",
    "quran": lambda v: v["collection"] == "Quran",
}


def select_verses(verses, scope: str = "christian", works: list[str] | None = None, language: str | None = None):
    """Filter verse records by named scope, language and/or explicit work codes."""
    if scope not in SCOPES:
        raise ValueError(f"unknown scope {scope!r}; choose from {sorted(SCOPES)}")
    pred = SCOPES[scope]
    wanted = {w.upper() for w in works} if works else None
    return [
        v for v in verses
        if pred(v) and (wanted is None or v["work"].upper() in wanted) and (language is None or v["language"] == language)
    ]
