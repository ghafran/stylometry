...
function styleSummary(rows){
 ...
 for(const row of rows){
  ...
  if(!groups.has(key))groups.set(key,{language:row.language,style_id:row.style_id,verse_count:0,token_count:0,books:new Set(),collections:new Set()});
  const group=groups.get(key);group.verse_count++;
 ...
 return Array.from(groups.values(),({books,collections,...group})=>({...group,book_count:books.size,collection_count:collections.size})).sort((a,b)=>str(a.language).localeCompare(str(b.language)) || b.verse_count-a.verse_count || str(a.style_id).localeCompare(str(b.style_id)));
}
