...
per style group under `authors/` and one page per work under `works/`. `output/index.html` links the
languages and `output/models/` holds the model comparison.
...
and the dashboard switch to a chapter breakdown (verse runs only: a pooled passage carries just its
first verse's chapter). For Hebrew the report also tabulates YHWH against Elohim per style group. That
comparison is not an independent check, because the character n-grams read the same text and the AI
style tags may name the divine names outright.
