"""Style works expose the selected corpus without losing hierarchy or text."""
import json
import shutil
import subprocess
...
def _rows():
    base = {"language": "eng", "collection": "C1", "book": "B1",
            ...
            "text": "A quiet morning", "style_id": "A", "status": "assigned",
    ...
    return [
        ...
        {**base, "id": "other", "style_id": "B", "token_count": 29},
        {**base, "id": "unassigned", "style_id": None, "token_count": 101},
    ]


def test_style_works_count_each_unit_once_across_the_complete_hierarchy():
    actual = _run(f"styleWorks({json.dumps(_rows())},'eng','A')", "styleWorks")
    assert {key: actual[key] for key in (
        "language", "style_id", "verse_count", "token_count", "book_count",
        "chapter_count", "collection_count"
    )} == dict(language="eng", style_id="A", verse_count=5, token_count=67,
...
def test_style_works_keep_language_namespaces_and_original_complete_verse_rows():
    expression = """(() => {
      const rows = ROWS, before = JSON.stringify(rows);
      const works = styleWorks(rows,'grc','A');
    ...
    actual = _run(expression, "styleWorks")
...
def test_style_works_respect_the_intersection_of_existing_corpus_filters(
    extra_filters, expected_ids, expected_words
):
    selection = dict(language="eng", collection="C1", book="B1", chapter="2", **extra_filters)
    expression = f"styleWorks(selectCorpus({json.dumps(_rows())},{json.dumps(selection)}),'eng','A')"
    actual = _run(expression, "selectCorpus", "styleWorks")
...
@pytest.mark.parametrize(("rows", "style"), [([], "A"), (_rows(), "absent"), (_rows(), None)])
def test_style_works_do_not_invent_works_for_empty_or_unassigned_styles(rows, style):
    actual = _run(f"styleWorks({json.dumps(rows)},'eng',{json.dumps(style)})", "styleWorks")
